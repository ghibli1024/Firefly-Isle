...
- 新建完整 Web 应用（当前项目完全空白，无任何代码）
- 引入自然语言 → 结构化数据提取能力（Claude API）
- 引入治疗时间线表格渲染与导出能力
...
- `editing`: 表格二次编辑——手动编辑字段、空白字段高亮提示
- `export`: PDF 导出（html2canvas + jsPDF）+ PNG 图片导出
- `llm-adapter`: 统一 LLM Adapter 接口，通过 Supabase Edge Function 代理，MVP 默认接 Claude API
...
- **外部依赖**：Supabase（Auth + PostgreSQL + Edge Functions + Storage）、Claude API
